...
# Main app layout - Birth date and time input
...
# Calculation date/time section
st.markdown("<div class='highlight'>", unsafe_allow_html=True)
st.markdown("<h2 class='sub-header'>📆 Calculation Reference Date</h2>", unsafe_allow_html=True)

# Option to use current date/time or custom date/time with a more visual toggle
...
if calculate_button:
    ...
    if birth_datetime > calculation_datetime:
        ...
        # Offer to show live counter
        st.markdown("</div>", unsafe_allow_html=True)
        
        if st.toggle("Show live age counter", value=False):
            st.warning("Live counter is running. Your age is being updated every second.")
            placeholder = st.empty()
            
            try:
                # Counter that runs for up to 1 minute or until user interaction
                for i in range(60):
                    current_time = datetime.datetime.now()
                    live_delta = relativedelta(current_time, birth_datetime)
                    
                    placeholder.markdown(f"""
                    <div style='text-align: center; font-size: 1.5rem; padding: 1rem; background-color: #e8f5e9; border-radius: 10px;'>
                        <span style='color: #2E7D32; font-weight: bold;'>{live_delta.years}</span> years, 
                        <span style='color: #2E7D32; font-weight: bold;'>{live_delta.months}</span> months, 
                        <span style='color: #2E7D32; font-weight: bold;'>{live_delta.days}</span> days, 
                        <span style='color: #2E7D32; font-weight: bold;'>{live_delta.hours}</span> hours, 
                        <span style='color: #2E7D32; font-weight: bold;'>{live_delta.minutes}</span> minutes, and 
                        <span style='color: #2E7D32; font-weight: bold;'>{live_delta.seconds}</span> seconds
                    </div>
                    """, unsafe_allow_html=True)
                    
                    time.sleep(1)
            except:
                st.error("Live counter stopped.")
